...
export function useSubscription(
	...
	React.useEffect(() => {
		...
		async function pollEvents(): Promise<void> {
			try {
				if (!pagingEntry.lastLedgerStart) {
					const latestLedgerState = await server.getLatestLedger()
					pagingEntry.lastLedgerStart = latestLedgerState.sequence
				}

				const lastLedger = pagingEntry.lastLedgerStart

				const response = await server.getEvents(
					pagingEntry.pagingToken
						? {
								cursor: pagingEntry.pagingToken,
								filters: [
									{
				...
				pagingEntry.pagingToken = undefined
				if (response.latestLedger) {
					pagingEntry.lastLedgerStart = response.latestLedger
				}
				if (response.events && response.events.length > 0) {
					...
					if (response.cursor) {
						pagingEntry.pagingToken = response.cursor
					}
				}
			} catch (error) {
